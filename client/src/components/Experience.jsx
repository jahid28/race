...
function Experience() {
  ...
  const userNameFromTokenFunc = useStore((state) => state.userNameFromTokenFunc);
  ...
  return (
    <>
      <main
        ref={menu}
        className="border-4 border-red-500 w-[100vw] h-[100vh] overflow-hidden absolute top-0 z-[99] grid place-items-center"
      >
...
<p>to change camera angle</p>
        ...
        <form
          onSubmit={submit}
          className="absolute formAnime bottom-12 left-1/2 -translate-x-1/2 grid place-items-center"
          action=""
        >
          {/* <input
            onChange={(e) => {
          ...
          <button
            type="submit"
            className="-skew-x-12 font-faster text-yellow-300 text-7xl border-2 border-yellow-300 hover:text-teal-950 hover:bg-yellow-300 cursor-pointer h-fit w-fit pt-.5 pb-1 px-8 transition-all duration-300"
          >
            START
          </button>
        </form>
